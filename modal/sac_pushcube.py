...
def train(task: str = "PushCube-v1"):
    ...
    envs = FlattenRGBDObservationWrapper(
        envs, rgb=use_rgb, depth=use_depth, state=INCLUDE_STATE
    )
    eval_envs = FlattenRGBDObservationWrapper(
        eval_envs, rgb=use_rgb, depth=use_depth, state=INCLUDE_STATE
    )
    ...
    envs = ManiSkillVectorEnv(
        envs, NUM_ENVS, ignore_terminations=True, record_metrics=True
    )
    eval_envs = ManiSkillVectorEnv(
        eval_envs, NUM_EVAL_ENVS, ignore_terminations=True, record_metrics=True
    )
